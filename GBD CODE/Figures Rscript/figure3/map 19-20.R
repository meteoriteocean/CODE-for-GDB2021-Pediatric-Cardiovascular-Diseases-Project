...
fill_in<-"NC-CVD\nIncidence % Change\n(2019-2020)"
file_name <- "IN Rate Change 2019-2020 plus"
...
Pop_num_2 <- pop_groups_iso3 %>%
  filter(year %in% c(2019), sex_name %in% "Both",age_group %in% "0") %>%
  mutate(Pop = est_val ) %>%
  dplyr::select(ISO.3.Code,year, Pop)
...
saveRDS(final_plot,file = here("/data_processed/", "fmap19_20.RDS"))

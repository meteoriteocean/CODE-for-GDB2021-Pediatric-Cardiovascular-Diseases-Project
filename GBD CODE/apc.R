apc2 <- function(R, ...)
{
  ...
  # (0) Fitted rates
  #
  ETA <- matrix(D$X%*%B, nrow=A, ncol=P) # the fitted age specific rates (log scale)
  ...
  v[v<0] <- NaN
  sd <- matrix(sqrt(v), nrow=A, ncol=P)
  EFit <- matrix(1/v, nrow=A, ncol=P)
  OFit <- matrix((1/v)*exp(-ETA), nrow=A, ncol=P)
  rates <- matrix(exp(ETA)*100000, nrow=A, ncol=P)
  CILo <- matrix(exp(ETA - 1.96*sd)*100000, nrow=A, ncol=P)
  CIHi <- matrix(exp(ETA + 1.96*sd)*100000, nrow=A, ncol=P)
...
APCFIT = function(PVP, X)  # fit the GLM model
{
  Y <- PVP$D$DATA[,4:5]; # extract events(4) and offset(5) data
  offset <- matrix(log(Y[,2])) # log scale of offset data
  y <- matrix(Y[,1]) # the matrix for events data
  ly <- log(y) # log scale of events data
  
  n <- nrow(X)  # X is the design matrix (i.e., columns of A, P, C data)
  p <- ncol(X)
  W <- y      # W <- the matrix of original events data 
  WX <- (W%*%matrix(1,ncol=p))*X    # W: events data in matrix; X: design matrix
  z <- (ly - offset)       # --> log(events) - log(offset) = log(events/offset)
  B <- solve(t(X)%*%WX,t(WX)%*%z)   # solve for the beta coefficients of the GLM
  V <- solve(t(X)%*%WX)     # the inverse of WX
  u <- matrix(Y[,2]*exp(X%*%B)) # the fitted events data (the prediction of model)
  wr2 <- matrix(W*(z-X%*%B)^2)    # 
  DEVRESIDS <- sign(y-u)*sqrt(wr2)   # calculate deviance residuals
  DEV <- sum(wr2)   # calculate the total deviance
  
  if (PVP$OverDispersion==1){
    s2 <- max(1, DEV/(n-p))  # s2 indicates the mean deviance
    }  else    {s2 <- 1}
  if (is.infinite(s2))
   {s2 <- 1}
  
  s2V = s2*V # what is s2VAR??
  APCMODEL <- list(B = B,  # beta coefficients
                   s2 = s2,
                   s2VAR = s2V, 
                   DEV = DEV, 
                   DevResids = DEVRESIDS)
...
designmatrix <- function(PVP)
{
  ...
  X <- cbind(J, a0, c0, Ad0[,2:(aM-1)], Pd0[,2:(pN-1)], Cd0[,2:(cK-1)]) # construct the design matrix
